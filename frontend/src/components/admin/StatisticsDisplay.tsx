...
export default function StatisticsDisplay({ statistics }: StatisticsDisplayProps) {
  ...
  const StatCard = ({
    ...
    <div className="bg-white border-2 border-black box-style-md p-2 sm:p-4">
      <div className="flex items-center justify-between">
        <div className="flex-1 min-w-0">
          <div className="text-[10px] sm:text-xs font-bold uppercase text-gray-600 mb-1 truncate">
            {title}
          </div>
          <div className="text-xl sm:text-2xl md:text-3xl font-black">{value.toLocaleString()}</div>
        </div>
        <div className={`${color} p-2 sm:p-3 border-2 border-black rounded-lg box-style-sm flex-shrink-0 ml-2`}>
  ...
  return (
    <div className="space-y-3 sm:space-y-5">
      <div>
        <h3 className="text-xs sm:text-sm font-bold uppercase tracking-wide mb-2 sm:mb-3
      ...
      <div className="bg-white border-2 border-black box-style-md p-2 sm:p-4">
        <h3 className="text-xs sm:text-sm font-bold uppercase tracking-wide mb-2 sm:mb-3 text-gray-700">
          Actions by Type
        </h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3">
          {Object.entries(statistics.audits.actionsByType).map(([type, count]) => (
            <div key={type} className="border-2 border-gray-300 p-2 sm:p-3 rounded box-style-sm">
              <div className="text-[10px] sm:text-xs text-gray-600 mb-1 truncate">{type}</div>
