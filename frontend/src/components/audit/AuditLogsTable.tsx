          ...
          <tr className="h-[40px] sm:h-[48px]">
            <th className="px-1 sm:px-2 text-left font-extrabold uppercase tracking-wider text-[10px] sm:text-xs">
              <div className="flex items-center gap-1">
                <Clock className="w-3 h-3 sm:w-4 sm:h-4 text-[#d46a5d]" />
                When
                <SortDesc className="w-2 h-2 sm:w-3 sm:h-3 text-xs opacity-70" />
              </div>
            </th>
            <th className="hidden md:table-cell px-2 text-center font-extrabold uppercase tracking-wider min-w-[70px] text-xs">
              <div className="flex items-center justify-center gap-1">
                <Activity className="w-4 h-4 text-[#40a8d0]" />
                Action
              </div>
            </th>
            <th className="px-2 sm:pl-12 sm:pr-2 text-left font-extrabold uppercase tracking-wider text-[10px] sm:text-xs">
              <div className="flex items-center gap-1">
                <FileText className="w-3 h-3 sm:w-4 sm:h-4 text-[#5cb973]" />
                Details
              </div>
            </th>
          </tr>
        ...
        <tbody>
          {logs.map((log, i) => (
            <tr
              key={log.id.toString()}
              className={`
                h-[48px] sm:h-[56px] transition-all
                ${i % 2 === 0 ? "bg-[#fffaf0]" : "bg-[#fff3e6]"}
                hover:bg-[#ffe9c2]
              `}>
              <td className="px-1 sm:px-2 py-1 align-middle">
                <div className="group inline-block">
                  <span className="text-[10px] sm:text-xs font-bold" aria-hidden>
                    {formatDateOnly(log.createdAt)}
                  </span>
                  <span
                    tabIndex={0}
                    className="ml-1 sm:ml-2 text-[10px] sm:text-xs opacity-0 group-hover:opacity-100 
                               group-focus:opacity-100 transition-opacity duration-150 focus:outline-none"
              ...
              <td className="hidden md:table-cell px-2 py-1 text-center align-middle">
              ...
              <td className="px-2 sm:pl-12 sm:pr-2 py-2 align-middle max-w-[48ch] break-words">
                <div className="text-xs sm:text-sm font-medium">{log.description || "No description"}</div>
                <div className="text-[10px] sm:text-xs text-gray-600 mt-1 flex gap-2">
                  <span>
